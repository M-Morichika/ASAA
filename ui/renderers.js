    ...
    <div class="grid two">
      ...
      <section class="section">
        ...
        <dl class="metric-list">
          ...
          <div class="metric-row"><dt>評価単位</dt><dd>企業戦略単位</dd></div>
      ...
      <div class="timeline-nodes">
        ${auditData.phases
          .map((phase, index) => {
            ...
            return `
              <article
                class="timeline-node selectable-card ${isSelected ? "is-selected" : ""}"
                data-timeline-phase="${phase.id}"
                role="button"
                tabindex="0"
                aria-pressed="${isSelected ? "true" : "false"}"
              >
                <h4>${index + 1}. ${phase.name}</h4>
                <dl>
...
function renderEvidence() {
  ...
  return `
    <div class="evidence-layout">
                      ...
                      .map(
                        (cell) => `
                          <button class="queue-row" type="button" data-queue-assessment-cell="${cell.id}">
                            ...
                            <span class="queue-action">経営判断監査で確認</span>
                          </button>
                        `,
                      )
                      .join("")}
      ...
      <section class="section detail-panel selected-detail-section">
        ${selectedLink
          ? `
...
function renderOpinion() {
  return `
    <section class="section">
      <h3>総合監査意見</h3>
      ${renderParagraphs(auditData.overviewOpinion)}
      <p><strong>限定事項:</strong> この監査意見は、証拠リンクに接続された証拠と未確認事項の範囲に限られる。</p>
    </section>
...
function renderPreWar() {
  ...
  return `
    <section class="section static-section">
      <h3>戦略表明前・大規模投資前に評価可能だった項目</h3>
      <p class="muted">「評価可能だった（可能性: 高〜中）のに公開資料上の評価形跡がない」ほど懸念が重い、という基準で算出。status は固定値ではなく評価可能性×評価形跡から導出される。</p>
      <div class="grid three">
        ${dist
          .map(
    ...
    ${
      ...
      <section class="section prewar-pending">
        <h3>未評価ギャップ</h3>
        <p class="muted">戦略表明前・大規模投資前に評価可能だったが、現在の証拠状態では評価形跡を断定できない項目。追加資料で「形跡あり/なし」が確認されるまで中立的に保留する。</p>
        ${pending
          .map(
            ({ item }) => `
              <div class="issue-row">
    ...
    <section class="section static-section">
      <h3>評価ギャップ・マトリクス</h3>
      <p class="muted">横軸=投資前の評価可能性、縦軸=公開資料上の評価形跡。右上（高 × 形跡なし）が重大懸念ゾーン。個別判断による評価は軸位置で配置せず、注釈バッジで重ねる。</p>
      ${matrix}
    </section>
