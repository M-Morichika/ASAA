...
import { cases } from "./data/cases/index.js?v=20260627-auto-ev-shift-r8";
import { createRenderers } from "./ui/renderers.js?v=20260627-auto-ev-shift-r8";
...
function renderCaseSelector() {
  const selector = document.querySelector("#case-selector");
  if (!selector) return;
  // 産業テーマ（conflict）単位で optgroup にまとめ、同一テーマの対照戦略を可視化する。
...
function selectTimelinePhase(phaseId) {
  state.activeTimelinePhaseId = phaseId;
  render();
  const phase = activeCase.phases.find((item) => item.id === state.activeTimelinePhaseId);
  if (phase) announce(`局面を選択: ${phase.name}`);
}
...
document.addEventListener("click", (event) => {
  ...
  if (timelinePhaseButton) {
    selectTimelinePhase(timelinePhaseButton.dataset.timelinePhase);
  }
  ...
  if (queueAssessmentButton) {
    ...
    announce(queuedCell ? `経営判断監査へ移動: ${queuedCell.axis} / ${queuedCell.phase}` : "経営判断監査へ移動");
  }
